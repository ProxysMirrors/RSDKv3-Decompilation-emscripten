...
static void ShutdownEngineRuntime()
{
    if (hasShutdown)
        return;
        ...
        switch (Engine.sdlEvents.type) {
            ...
            case SDL_WINDOWEVENT:
                switch (Engine.sdlEvents.window.event) {
                    ...
                    case SDL_WINDOWEVENT_SIZE_CHANGED: Engine.windowRefreshTimer = 3; break;
            ...
            case SDL_KEYDOWN:
                ...
                switch (Engine.sdlEvents.key.keysym.sym) {
                    ...
                    case SDLK_PAUSE:
                        if (Engine.devMenu)
                            Engine.masterPaused = !Engine.masterPaused;
                        break;
...
void RetroEngine::Init()
{
    ...
    if (!webStorageInitialised) {
        return; // wait for storage to mount
    }
    LoadUserdata();
...
#endif
    PrintLog("DevMenu: %s, DebugMode: %s", Engine.devMenu ? "ON" : "OFF", engineDebugMode ? "ON" : "OFF");

    Engine.startList_Game  = Engine.startList;
    Engine.startStage_Game = Engine.startStage;
    CalculateTrigAngles();
    GenerateBlendLookupTable();
    PrintLog("Engine Init started (Storage Ready)");
    PrintLog("Data file: %s", Engine.dataFile);
#else
    CalculateTrigAngles();
    GenerateBlendLookupTable();
    PrintLog("Engine Init started");
    InitUserdata();
#endif
...
#endif
    if (!Engine.usingDataFile) {
        char fallbackDest[0x200];
        StrCopy(fallbackDest, "Data/");
        StrAdd(fallbackDest, Engine.dataFile);
        CheckRSDKFile(fallbackDest);
    }
    ...
    if (LoadGameConfig("Data/Game/GameConfig.bin")) {
        PrintLog("GameConfig loaded successfully");
        if (InitRenderDevice()) {
            PrintLog("RenderDevice initialised successfully");
            if (InitAudioPlayback()) {
                PrintLog("AudioPlayback initialised successfully");
                InitFirstStage();
                ClearScriptData();
                initialised = true;
                running     = true;
                ...
                PrintLog("AudioPlayback initialisation FAILED");
            }
        }
        else {
            PrintLog("RenderDevice initialisation FAILED");
        }
    }
    else {
        PrintLog("LoadGameConfig FAILED");
    }
    ...
    if (!running) {
        ...
        if (usingCWD)
            ...
            sprintf(rootDir, "%s", BASE_PATH);

        sprintf(pathBuffer, "%sData.rsdk", rootDir);
        FileIO *file = fOpen(pathBuffer, "rb");
        if (!file) {
            sprintf(gameWindowText, "Data.rsdk Not Found");
            sprintf(gameDescriptionText, "Please place Data.rsdk in the following directory:\n%s\n\nThen restart the game.", rootDir);
        }
        else {
            fClose(file);
            sprintf(gameWindowText, "GameConfig.bin Not Found");
            sprintf(gameDescriptionText,
                    "Please extract the Data folder from Data.rsdk into the following directory:\n%s\n\nThen restart the game.", rootDir);
        }
        InitErrorMessage();
        initialised = true;
        running     = true;
...
#if RETRO_PLATFORM == RETRO_WEB
static void MainLoopWeb(void *userData)
{
    RetroEngine *engine = (RetroEngine *)userData;

    if (!webStorageInitialised) {
        // Wait for storage to mount
        static int waitFrames = 0;
        if (++waitFrames % 60 == 0) {
            PrintLog("Waiting for web storage... (%d)", waitFrames);
        }
        return;
    }
    ...
    if (!engine->running) {
        ...
        ShutdownEngineRuntime();
        return;
    }

    // Frame skip guard for high refresh rate displays (120Hz+)
    static double lastFrameTime = 0;
    double now                  = emscripten_get_now();
    double elapsed              = now - lastFrameTime;
    if (elapsed < (1000.0 / 60.0) - 1.0)
        return;
    lastFrameTime = now;

    engine->running = ProcessEvents();

    if (engine->windowRefreshTimer > 0) {
        RefreshWindow();
        engine->windowRefreshTimer--;
    }
        ...
        if (!engine->hasFocus) {
            if (!(engine->focusState & 1)) {
                ...
                if (engine->gameMode == ENGINE_MAINGAME) {
                    if (activeStageList != STAGELIST_SPECIAL && activeStageList != STAGELIST_PRESENTATION) {
                        engine->gameMode = ENGINE_PAUSE;
                    }
                }
            }
        }
            ...
            if ((engine->focusState & 2))
                ResumeSound();
            engine->focusState = 0;
            if (engine->gameMode == ENGINE_PAUSE)
                engine->gameMode = ENGINE_MAINGAME;
        }
    }
        ...
        for (int s = 0; s < engine->gameSpeed; ++s) {
            ...
            if (!engine->masterPaused || engine->frameStep) {
                switch (engine->gameMode) {
                    ...
                    case ENGINE_EXITGAME: engine->running = false; break;
                    ...
                    case ENGINE_WAIT:
                        if (engine->waitValue > 0)
                            engine->waitValue--;
                        else
                            engine->gameMode = engine->prevMessage;
                        break;
...
{
#if RETRO_PLATFORM == RETRO_WEB
    InitUserdata(); // Trigger storage mount
    emscripten_set_main_loop_arg(MainLoopWeb, this, 0, 1);
#else
    while (running) {
            ...
            switch (sdlEvents.type) {
                ...
                case SDL_WINDOWEVENT:
                    switch (sdlEvents.window.event) {
                        ...
                        case SDL_WINDOWEVENT_RESIZED:
                        case SDL_WINDOWEVENT_SIZE_CHANGED: windowRefreshTimer = 3; break;
                        case SDL_WINDOWEVENT_CLOSE: gameMode = ENGINE_EXITGAME; break;
                        case SDL_WINDOWEVENT_FOCUS_LOST:
                            if (!((disableFocusPause + 1) & 1))
                                Engine.message = MESSAGE_LOSTFOCUS;
                            hasFocus = false;
                    ...
                    break;
                case SDL_CONTROLLERDEVICEADDED: ControllerInit(sdlEvents.cdevice.which); break;
                case SDL_CONTROLLERDEVICEREMOVED: ControllerClose(sdlEvents.cdevice.which); break;
                case SDL_APP_WILLENTERBACKGROUND:
                    if (!((disableFocusPause + 1) & 1))
                        Engine.message = MESSAGE_LOSTFOCUS;
                    hasFocus = false;
                    break;
                case SDL_APP_WILLENTERFOREGROUND: hasFocus = true; break;
                case SDL_APP_TERMINATING: gameMode = ENGINE_EXITGAME; break;
...
#if RETRO_USING_OPENGL
                        SDL_GL_GetDrawableSize(window, &pixW, &pixH);
#else
                        SDL_GetRendererOutputSize(renderer, &pixW, &pixH);
#endif
                ...
                case SDL_FINGERDOWN:
                    TryResumeAudioDevice();
                case SDL_FINGERUP: {
                    int count = SDL_GetNumTouchFingers(sdlEvents.tfinger.touchId);
                    touches   = 0;
                    for (int i = 0; i < count; i++) {
                        SDL_Finger *finger = SDL_GetTouchFinger(sdlEvents.tfinger.touchId, i);
                        if (finger && touches < 8) {
                            touchDown[touches] = true;
                            touchX[touches]    = finger->x * SCREEN_XSIZE;
                            touchY[touches]    = finger->y * SCREEN_YSIZE;
...
#if RETRO_USE_MOD_LOADER
                                // hacky patch because people can escape
                                if (gameMode == ENGINE_DEVMENU && stageMode == DEVMENU_MODMENU) {
                                    RefreshEngine();
                                }
#endif
                ...
                case SDL_KEYUP:
                    switch (sdlEvents.key.keysym.sym) {
                        default: break;
                        case SDLK_BACKSPACE: gameSpeed = 1; break;
                    }
                    break;
        ...
        if (vsync) {
            double targetMS = 1000.0 / targetRefreshRate;
            double elapsed  = SDL_GetTicks() - startTime;
            if (elapsed < targetMS) {
                Uint32 delay = (Uint32)(targetMS - elapsed);
                if (delay > 1)
                    SDL_Delay(delay - 1); // Sleep most of the remaining time

                while (SDL_GetTicks() - startTime < targetMS) {
                    // Busy-wait for the final <1ms for better precision
                }
            }
        }
...
void RetroEngine::Callback(int callbackID)
{
    ...
    switch (callbackID) {
        ...
        case CALLBACK_GETWINDOWINFO:
            ...
            SetGlobalVariableByName("Engine.HardwareRenderer", StrComp(Engine.gameRenderType, "HW_Rendering"));            
            break;
